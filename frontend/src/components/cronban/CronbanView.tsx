 ...
 *   Helpers   — helper session pool (claude_code evaluator sessions)
 */
import { Tabs, TabsContent, TabsList, TabsTrigger } from '@/components/ui/tabs';
import { Calendar, Kanban, Webhook, BookOpen, ShieldCheck, Bot } from 'lucide-react';
  ...
  return (
    <Tabs defaultValue="kanban" className="h-full flex flex-col">
      <TabsList className="shrink-0 rounded-none border-b border-border bg-transparent px-4 justify-start gap-1 h-10">
        <TabsTrigger value="calendar" className="h-8 gap-1.5 text-xs data-[state=active]:bg-muted">
          <Calendar className="h-3.5 w-3.5" />
          Calendar
        </TabsTrigger>
        <TabsTrigger value="kanban" className="h-8 gap-1.5 text-xs data-[state=active]:bg-muted">
          <Kanban className="h-3.5 w-3.5" />
          Kanban
        </TabsTrigger>
        <TabsTrigger value="webhooks" className="h-8 gap-1.5 text-xs data-[state=active]:bg-muted">
          <Webhook className="h-3.5 w-3.5" />
          Webhooks
        </TabsTrigger>
        <TabsTrigger value="skills" className="h-8 gap-1.5 text-xs data-[state=active]:bg-muted">
          <BookOpen className="h-3.5 w-3.5" />
          Skills
        </TabsTrigger>
        <TabsTrigger value="gates" className="h-8 gap-1.5 text-xs data-[state=active]:bg-muted">
          <ShieldCheck className="h-3.5 w-3.5" />
          Gates
        </TabsTrigger>
        <TabsTrigger value="helpers" className="h-8 gap-1.5 text-xs data-[state=active]:bg-muted">
      ...
      <TabsContent value="helpers" className="flex-1 overflow-hidden mt-0">
        <HelperSessionsPanel />
      </TabsContent>
    </Tabs>
  );
